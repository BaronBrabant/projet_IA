...
def give_orders(game_status):
    ...
    * = attack
    ...
    if game_status == False:
        ...
        order_string_2 = input('Player 2 input orders') # <--- input in another function difficulty playing ai 
        if orders_string != '':
...
def ant_attack (ant_dict, list_orders, player):
    ...
    if player == '2':
        ...
        for counter in range(1, amount_ants+1):
            print(ant_dict['Ant-%s'%counter]['health'] , 'this is what the counter ext6racts for healthj')
            
            if ant_dict['Ant-%s'%counter]['health'] <= 0:
                print('THIS MEANS IT SHOULD BE DELETED')
                square_id_del = ant_dict['Ant-%s'%counter]['square_id']
                square_id_del -= 1
                if map_position[square_id_del][3] != None:
                    if ant_dict['Ant-%s'%counter]['earth'] != 0:
                        earth_weight = ant_dict['Ant-%s'%counter]['earth']
    ...
    if player == '1':
        for legal_order in legal_orders:
            ...
            if origin_coord not in used_orders:
                if map_position[square_id_destination][3] == None and map_position[square_id_destination][4] == 0:
...
def ant_level(anthills_square_ids):
    ...
    print(level_ant_2, 'ceci est le niveau de la fourmis')
...
while game_status == False:
    ...
    print('health ant 2', ant_dict['Ant-2']['health'])
    print('health ant 1', ant_dict['Ant-1']['health'])
